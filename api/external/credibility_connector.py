...
def get_source_credibility(source):
    response = requests.get(
        f"{CREDIBILITY_ENDPOINT}/sources/", params={"source": source}
    )
    if response.status_code != 200:
        raise ValueError(response.status_code)
    return response.json()
...
def post_source_credibility_multiple(sources):
    response = requests.post(
        f"{CREDIBILITY_ENDPOINT}/sources/", json={"sources": sources}
    )
    if response.status_code != 200:
        raise ValueError(response.status_code)
    return response.json()
...
def update_origin(origin_name: str):
    response = requests.post(f"{CREDIBILITY_ENDPOINT}/origins/{origin_name}")
