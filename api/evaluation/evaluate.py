...
def count_user(user, tweets, allow_cached, only_cached, use_credibility):
    ...
    if len(fake) + len(verified):
        score = (50.0 * (1 * len(verified) + -10 * len(fake))) / (
            10 * len(fake) + 5 * len(mixed) + len(verified)
        ) + 50
...
def classify_urls_credibility(urls_info):
    ...
    urls_to_domains = {
        el["resolved"]: utils.get_url_domain(el["resolved"]) for el in urls_info
    }
    domains_credibility = credibility_manager.get_sources_credibility(
        list(set(urls_to_domains.values()))
    )
    ...
    for url_info in urls_info:
        ...
        if abs(domain_credibility["credibility"]["confidence"]) < 0.2:
            ...
            label = {
                ...
                "score": {
                    "label": "true"
                    if (credibility_value > 0.4)
                    else ("fake" if (credibility_value < -0.4) else "mixed")
                },
                "reason": "domain_match",
                "url": url,
...
def get_factchecking_by_domain():
    ...
    by_fact_checker_domain = itertools.groupby(
        sorted(
            all_factchecking,
            key=lambda el: utils.get_url_domain_without_subdomains(el["url"]),
        ),
        key=lambda el: utils.get_url_domain_without_subdomains(el["url"]),
    )
...
def get_factchecking_by_factchecker():
    ...
    result = {
        el["domain"]: {"name": el["name"], "fact_checking": []} for el in fact_checkers
    }
    ...
    for k, v in result.items():
        ...
        fact_checking_matching_with_claim_url = [
            el for el in fact_checking_matching if el.get("claim_url", None)
        ]
...
def get_factchecking_by_one_domain(domain, time_granularity="month"):
    ...
    for fcu in tqdm.tqdm(values_with_claim_url):
        ...
        tweet_ids_sharing_factchecking = data.get_tweets_containing_url(
            factchecking_url
        )
        ...
        if not (factchecking_url in urls):
            overall_counts["factchecking_shares_count"] += len(
                tweet_ids_sharing_factchecking
            )
            overall_counts["by_label"][fcu["label"] or "unknown"][
                "factchecking_shares_count"
            ] += len(tweet_ids_sharing_factchecking)
            urls.add(factchecking_url)
        if not (claim_url in urls):
            overall_counts["claims_shares_count"] += len(tweet_ids_sharing_claim)
            overall_counts["by_label"][fcu["label"] or "unknown"][
                "claims_shares_count"
            ] += len(tweet_ids_sharing_claim)
            urls.add(claim_url)
...
def analyse_url_distribution(url, reference_date=None, time_granularity="month"):
    ...
    if reference_date:
        ...
        for k, v in analyse_tweet_time_relative(
            None, tweet_ids_sharing, time_granularity
        ).items():
            tweets_relative[k] += v
        result = tweets_relative
    else:
        # absolute analysis
        result = analyse_tweet_time(
            tweet_ids_sharing, time_granularity, "absolute", None
        )
