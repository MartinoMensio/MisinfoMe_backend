...
def post_source_credibility(
    ...
    callback_url: str = Query(
        None,
        description="The callback_url coming from the gateway. If absent, the call will be blocking",
    ),
):
    """
    Get the credibility of a certain source
...
def post_url_credibility(
    ...
    if callback_url:
        return jobs_manager.create_task_for(
            credibility_manager.get_url_credibility, url, callback_url=callback_url
        )
    else:
        return credibility_manager.get_url_credibility(url)
...
def get_tweet_credibility(
    ...
    if not wait:
        return jobs_manager.create_task_for(
            credibility_manager.get_tweet_credibility_from_id, tweet_id
        )
    else:
...
def post_tweet_credibility(
    ...
    if callback_url:
        return jobs_manager.create_task_for(
            credibility_manager.get_tweet_credibility_from_id,
            tweet_id,
            callback_url=callback_url,
        )
    else:
...
def post_tweet_credibility_batch(
    tweets: List[dict] = Body(
        ...,
        description="A list of tweets, each one being a dict with the keys `id` and `text`",
    )
):
...
def get_user_credibility(
    ...
    if not wait:
        return jobs_manager.create_task_for(
            credibility_manager.get_user_credibility_from_screen_name, screen_name
        )
    else:
...
def post_user_credibility(
    ...
    callback_url: str = Query(
        None,
        description="A URL to be POSTed with the result of the job. If absent, the call will be blocking",
    ),
):
    """Endpoint for gateway"""
    if callback_url:
        return jobs_manager.create_task_for(
            credibility_manager.get_user_credibility_from_screen_name,
            screen_name,
            callback_url=callback_url,
        )
    else:
...
def get_user_friends_credibility(
    screen_name: str = Query(
        ...,
        description="The `screen_name` of the twitter profile with the friends to get the cached analysis",
    ),
    limit: int = Query(300, description="How many friends to retrieve, default 300"),
):
    """Get the credibility of a certain user"""
    result = credibility_manager.get_user_friends_credibility_from_screen_name(
        screen_name, limit
    )
    return result
