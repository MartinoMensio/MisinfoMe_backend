...
def get_tweets(
    ...
    cached_only: bool = Query(False, description="Get only tweets that are already in the cache"),
...
def get_factchecking_organisation(org_id: str = Path(..., description="The id of the factchecking organisation to look for")):
...
def get_factchecking_organisations(belongs_to_ifcn: bool = None, valid_ifcn: bool = None, country: str = None):
...
def get_factchecking_reviews(
    published_by_id: str = Query(None, description="Get the factchecking reviews published by a single organisation (identifier)"),
    published_at_domain: str = Query(None, description="Get the factchecking reviews published on a certain domain"),
    published_at_url: str = Query(None, description="Get the factchecking reviews published on a certain URL")
    ):
    if published_by_id:
