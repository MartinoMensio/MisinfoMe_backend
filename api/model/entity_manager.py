...
def get_tweets_containing_url(
    url, cached_only=False, from_date=None, limit=None, offset=0
):
...
def get_factchecking_reviews_from_organisation_id(
    factchecking_organisation_id, from_date=None
):
...
def get_latest_reviews(max=6):
    ...
    for el in latest:
        ...
        description = (
            el["goose"]["opengraph"].get("description", None)
            or el["goose"]["meta"]["description"]
        )
