...
GATEWAY_MODULE_ENDPOINT = os.environ.get(
    "GATEWAY_MODULE_ENDPOINT", "https://localhost:1234/foo"
)
...
class CallbackTask(Task):
    def on_success(self, retval, job_id, args, kwargs):
        ...
        response_object = {"response": retval}
        if not "api.coinform.eu/" in callback_url:
            print("callback_url not valid")
            return
        try:
            headers = {"Authorization": f"Bearer {COINFORM_TOKEN}"}
            response = requests.post(
                callback_url, json=response_object, headers=headers, verify=False
            )
...
)  # TODO(bind=True) will have self.update_state() to let the user to have more details https://blog.miguelgrinberg.com/post/using-celery-with-flask
...
def create_task_for(*args, **kwargs):
    ...
    return {"job_id": job.id, "callback_url": callback_url}
...
def get_task_status(job_id):
    ...
    if task.state == "PENDING":
        ...
        response = {
            ...
            "status": str(
                task.info
            ),  # this is the exception raised # TODO get status_code propagated and detail attribute
            "error": {},
        }
